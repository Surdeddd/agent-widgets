...
- `aw preview <id> --built-in-sizes` renders at the built-in widget sizes instead of the ones measured on this Mac, which is what CI and a Mac that has not measured yet will see. The 2048 and system pulse widgets size themselves from the space they get, so they fit both.
- `SAMPLE_NOT_LOCALIZED` fires only for feeds that read `AW_LANG`: a feed that prints the same text in every language has nothing to translate.
- System date text follows the widget language: the kit hands `context.locale` to SwiftUI, so `Text(date, style:)` and `Text(date, format:)` stop printing Russian weekdays inside an English widget.
- `AWSparkline(values, range: 0...100)` pins the vertical scale, so a 5 % move of a percentage no longer looks like a cliff.
